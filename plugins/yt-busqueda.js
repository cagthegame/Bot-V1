...
${Prefijo+t} cuanto es 1 + 1`);let r;await a.reply(MultiNK.Bsqd(await e.getName(a.sender)));let l=encodeURIComponent(n);try{let s=await fetchJson(`https://latam-api.vercel.app/api/yts?apikey=${MyApiKey}&q=${l}`),i="*[ > ] Resultados en Youtube para:* _"+n+"_\n\n";for(let u of s.resultados)i+=`🔖 Titulo: ${u.titulo}
🧬 ID: ${u.id}
⛓️ URL: ${u.url}
🗜️ Tipo: ${u.tipo}
